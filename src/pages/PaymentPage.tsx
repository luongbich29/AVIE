...
export default function PaymentPage() {
  ...
  return (
    <div className="bg-background text-on-background min-h-screen flex flex-col font-sans relative antialiased select-none">
      ...
      <header className="fixed top-0 w-full z-50 bg-background/95 backdrop-blur border-b border-outline-variant">
      ...
      {/* Main Content Canvas */}
      <main className="flex-grow pt-24 pb-32 px-container-padding max-w-screen-md mx-auto w-full">
        {/* Summary Section */}
        <div className="mb-8">
          <h2 className="font-headline-md text-headline-md text-primary mb-3">Lựa chọn thanh toán</h2>
          ...
          {options.map((option) => {
            ...
            return (
              <button
                ...
                className={[
                  "w-full text-left relative flex items-center p-4 border rounded-lg transition-all duration-200 group active:scale-[0.99]",
                  isActive
              ...
              >
                <div className="flex items-center w-full">
                  ...
                  <div
                    className={[
                      "w-10 h-10 flex items-center justify-center bg-surface-container rounded-lg mr-4 transition-colors overflow-hidden shrink-0",
                      isActive ? "bg-secondary-fixed text-secondary" : "group-hover:bg-secondary-fixed",
                    ].join(" ")}
                  >
                    {option.iconType === "icon" ? (
                      <span className="material-symbols-outlined text-primary text-2xl">{option.iconName}</span>
                    ) : (
                      <img alt={option.title} className="w-8 h-8 rounded-md object-contain" src={option.iconSrc} />
                    )}
                  </div>
                  ...
                  <div
                    className={[
                      ...
                      isActive
                        ? "border-secondary bg-secondary shadow-[inset_0_0_0_3px_#ffffff]"
      ...
      <div className="fixed bottom-0 w-full bg-background border-t border-outline-variant p-4 z-50">
        <div className="max-w-screen-md mx-auto">
          <button
            ...
            disabled={isProcessing}
            className={[
              "w-full bg-primary text-on-primary py-4 rounded font-label-md text-label-md uppercase tracking-widest transition-all shadow-md select-none font-bold",
              isProcessing
                ? "opacity-80 cursor-not-allowed"
                : "active:scale-[0.98] hover:bg-primary/95 hover:shadow-lg",
            ].join(" ")}
          >
